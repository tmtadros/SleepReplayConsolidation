%% fashion MNIST catastrophic forgetting example, GIDO Class task
%    Load paths
clear all; close all;
...
save('fashionmnist_accuracy_optimized', 'accuracy_sleep', 'accuracy_nosleep')

%% Plot accuracy
% figure;
% num_replayed_examples = [0 100 200 300 400 500 1000 5000];
% error = std(accuracy, [], 2);
% hE= errorbar(num_replayed_examples, mean(accuracy,2), error); hold on
% set(hE                            , ...
%   'Marker'          , '.'         , ...
%   'Color'           , [.3 .3 .3]  );
% set(hE                            , ...
...
% 
% hTitle  = title ('Sleep with Replay    _');
