function [ f ] = confmat_basic( nn, Snn, testX, testY)
...
f = figure();
subplot(1,2,1)
imshow(awakeConfMat); hold on;
caxis([0 1100]);

colormap(flipud(gray));  % Change the colormap to gray (so higher values are
                         %   black and lower values are white)

%textStrings = num2str(awakeConfMat(:));       % Create strings from the matrix values
...
subplot(1,2,2)
imshow(sleepConfMat); hold on;

colormap(flipud(gray));  % Change the colormap to gray (so higher values are
                         %   black and lower values are white)
caxis([0 1100]);

%textStrings = num2str(sleepConfMat(:));       % Create strings from the matrix values
%textStrings = strtrim(cellstr(textStrings));  % Remove any space padding
%hStrings = text(x(:), y(:), textStrings(:), ...  % Plot the strings
%                'HorizontalAlignment', 'center');
%midValue = mean(get(gca, 'CLim'));  % Get the middle value of the color range
