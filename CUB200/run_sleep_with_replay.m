function [ ac1, ac2 ] = run_sleep_with_replay(percent_replay, do_sleep)
...
inds2 = 1:length(task1_train_x);
num_replayed_examples = round(length(task1_train_x)*percent_replay,-2)
inds2 = inds2(randperm(length(inds2)));
tr_x = vertcat(task2_train_x(1:2900,:), task1_train_x(inds2(1:num_replayed_examples),:));
tr_y = vertcat(task2_train_y(1:2900,:), task1_train_y(inds2(1:num_replayed_examples),:));
nn1 = nntrain(nn1, tr_x, tr_y, opts);
nn_pointer = nn1;
...
if do_sleep == 1    
    ...
    [norm_nn, norm_constants] = normalize_nn_data(nn_pointer, train_x);
    sleep_opts.alpha = norm_constants*alpha_scale;
    sleep_input = repmat(mean(train_x), sleep_dur,1)'; 

    Snn = sleepnn_old(nn_pointer, sleep_dur, t_opts, sleep_opts, ...
...
er1 = nntest(Snn, task1_test_x, task1_test_y);
er2 = nntest(Snn, task2_test_x, task2_test_y);
